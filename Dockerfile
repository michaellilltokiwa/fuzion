...
RUN apt-get update && apt-get -y --no-install-recommends install locales openjdk-21-jdk-headless git make patch libgc1 libgc-dev shellcheck asciidoc asciidoctor ruby-asciidoctor-pdf antlr4 clang-18 wget ditaa inkscape unzip openssh-client && ln -s /usr/bin/clang-18 /usr/bin/clang
