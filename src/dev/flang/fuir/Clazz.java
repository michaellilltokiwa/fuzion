...
class Clazz extends ANY implements Comparable<Clazz>
{
   ...
   */
  private boolean _isInstantiatedChoice = true; // NYI: HACK: false;
  ...
  boolean isUnitType()
  {
    ...
    if (_fuir._lookupDone &&
        ...
        !isChoice())
      {
        ...
        //
        // Object layout will later report an error for this case. (NYI: DOCUMENTATION: check this with a test!)
  ...
  /**
   * find redefinition of a given feature in this clazz. NYI: UNDER DEVELOPMENT: This will have to
    ...
    if (f != Types.f_ERROR && tf != Types.resolved.f_void && chain != null)
      {
        for (var p: chain)
          {
            fn = f.outer().handDown(null, f, fn, p, feature());  // NYI: BUG: need to update f/f.outer() to support several levels of inheritance correctly!
          }
      }
 ...
 */
    ...
    if (result == null)
      {
        ...
        // NYI: PERFORMANCE: would be good if postFeatures could be identified not be string comparison, but with something like
        ...
        if (result == 0)
          {
            if (to.isRef() && oo.isRef())
              { // NYI: CLEANUP: If outer is normalized for refs as described in the
  ...
  public int hashCode()
  {
    return (_type.isRef() ? 0x777377 : 0) ^ feature().globalIndex();  // NYI: BUG: outer and type parameters!
  }
  ...
  private boolean isOuterInstantiated()
  {
    ...
    return o == null ||
      ...
      // NYI: UNDER DEVELOPMENT: Once Clazz.normalize() is implemented better, a clazz C has
