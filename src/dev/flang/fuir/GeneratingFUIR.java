...
public class GeneratingFUIR extends FUIR
{
  ...
  /**
   * Functional interface to replace intrinsics.
   */
  @FunctionalInterface
  interface RemoveIntrinsic
  {
    Expr code(AbstractCall c, Clazz currentClazz, GeneratingFUIR fuir);
  }
  ...
  /**
   * For a call to {@code autogenerated_code_for_Values_Of_Open_Type_fold}/{@code autogenerated_code_for_Values_Of_Open_Type_zip},
   * extract the fields that we are operating on.
   *
   * @param currentClazz the current clazz, i.e., the {@code fold} or {@code zip} operation
   *
   * @return the list of fields in the order corresponding to the order of actual type parameters
   */
  private static java.util.List<Clazz> openTypeFields(Clazz currentClazz)
  {
    ...
    // the number of detected clazzes and fields are still growing (this
    ...
    if (CHECKS) check
      (// we expect that `fields` come in the order of their `_select`
       IntStream.range(0, fields.size()).allMatch(i -> i == fields.get(i)._select));
  ...
  static
  {
    _removedIntrinsics_.put
      ("autogenerated_code_for_Values_Of_Open_Type_fold",
       (call, currentClazz, fuir) ->
       {
    ...
    _removedIntrinsics_.put
      ("autogenerated_code_for_Values_Of_Open_Type_zip",
       (call, currentClazz, fuir) ->
       {
         Expr other = call.actuals().get(0);
         Expr e = call.actuals().get(1);
         var zz = call.actuals().get(2);
         for (var f : openTypeFields(currentClazz))
           {
             var t = f.resultClazz()._type;
             var ta = currentClazz.actualTypeParameters()[1];
             ...
             var other_featureWithOpenTypeParamter = new AbstractCall()  // e.g. tuple from other.outer
               {
                 @Override public SourcePosition     pos()                  { return call.pos(); }
                 @Override public Expr               target()               { return other; }
                 @Override public AbstractFeature    calledFeature()        { return currentClazz._outer.feature().outerRef(); }
                 @Override public AbstractType       type()                 { return currentClazz._outer.feature().outerRef().resultType(); }
               };
             var other_field = new AbstractCall()                  // e.g. other.outer.values.3
               {
                 @Override public SourcePosition     pos()                  { return call.pos(); }
                 @Override public Expr               target()               { return other_featureWithOpenTypeParamter; }
                 @Override public AbstractFeature    calledFeature()        { return f.feature(); }
                 @Override public int                select()               { return f._select; }
                 @Override public AbstractType       type()                 { return t; }
               };
             var fe = e; // effectively final copy of e
             e = new AbstractCall()                                // e.g. zz.apply <type of tuple.values.3> e tuple.values.3 other.outer.values.3
               {
                 @Override public SourcePosition     pos()                  { return call.pos(); }
    ...
    _removedIntrinsics_.put
      ("autogenerated_code_for_Open_Types_fold",
       (call, currentClazz, fuir) ->
       {
         if (CHECKS) check
           (currentClazz.feature() == Types.resolved.f_Open_Types_type_foldf);  // make sure noone tries to call this in a different context
         ...
         return codeForTypeFold(call,
                                currentClazz,
                                currentClazz.replaceOpen(openType, openTypePar));   // for all actual types assigned to the open type parameter
       });

    _removedIntrinsics_.put
      ("autogenerated_code_for_effect_types_fold",
       (call, currentClazz, fuir) ->
       {
         fuir._clazzesWhoseCodeDependsOnUsedEffectTypes.add(currentClazz);

         if (CHECKS) check
           (currentClazz.feature() == Types.resolved.f_effect_types_type_foldf);// make sure noone tries to call this in a different context
  ...
  /**
   * Get the names of all intrinsics that get removed by the middle end.
   */
  public static Set<String> removedIntrinsics()
  {
    return _removedIntrinsics_.keySet();
  }


  /**
   * Helper method used by _removedIntrinsics for
   * {@code autogenerated_code_for_Open_Types_fold} and
   * {@code autogenerated_code_for_effect_types_fold} to create the actual inline
   * code.
   *
   * @param call to the {@code autogenerated_code_for...} method we are creating code
   * for.
   *
   * @param currentClazz the surrounding clazz that performs the call.
   *
   * @param types the types that are to be folded
   *
   * @return the actual code
   */
  private static Expr codeForTypeFold(AbstractCall call,
                                      Clazz currentClazz,
                                      List<AbstractType> types)
  {
    Expr effect = call.actuals().get(0);
    var aa = call.actuals().get(1);
    for (var t : types)
      {
        var ta = currentClazz.actualTypeParameters()[1];
        var apply = ta.lookup(new FeatureAndActuals(Types.resolved.f_type_applicator_apply,
                              ...
                              FuzionConstants.NO_SELECT,
                              false /* isInheritanceCall */);
        var fe = effect; // effectively final copy of effect
        effect = new AbstractCall()                                // e.g. aa.apply <type of tuple.values.3> e
          {
            @Override public SourcePosition     pos()                  { return call.pos(); }
            @Override public Expr               target()               { return aa; }
            @Override public AbstractFeature    calledFeature()        { return apply.feature(); }
  ...
  /**
   * All the clazzes of effect types that ever get instated.
   */
  final TreeSet<Integer>   _usedEffectTypes = new TreeSet<>();


  /**
   * All the clazzes whose code depends on _usedEffectTypes. Their code
   * must be re-generated in case new effect clazzes are found to be instated.
   */
  final TreeSet<Clazz>     _clazzesWhoseCodeDependsOnUsedEffectTypes = new TreeSet<>();
  ...
  void addCode(int cl, Clazz c, List<Object> code, List<List<AbstractCall>> inhe, LibraryFeature ff, List<AbstractCall> inh)
  {
    ...
    if (CHECKS) check
      (_currentClazz == null || _currentClazz == c); /* make sure no recursive call changes this */
    _currentClazz = c;
   ...
   *
   * This is needed during {@code toStack} when replacing intrinsics by code.
   */
        ...
        if (e instanceof AbstractCall call)
          {
            ...
            if (cf.isIntrinsic() && _removedIntrinsics_.containsKey(qn))
              {
                e = _removedIntrinsics_.get(qn).code(call, _currentClazz, this);
              }
   ...
   * {@inheritDoc}
   *
   * This redefinition additionally tracks all instated effect types for use in
   * {@code effect_types.autogenerated_code_for_effect_types_fold}.
   *
   * In case a new type is added, all the code that was generated for
   * {@code effect_types.autogenerated_code_for_effect_types_fold} will be deleted.
   */
  @Override
  public int effectTypeFromIntrinsic(int cl)
  {
    ...
    var res = super.effectTypeFromIntrinsic(cl);
  ...
  /**
   * Add the given effect type to _usedEffectTypes and make sure that code
   * for all clazzes in _clazzesWhosecodedependsoninstatedeffecttypes will be
   * re-generated since these clazzes' code iterates over all the
   * {@code _usedEffectTypes}.
   *
   * @param cl the class of the effect type that is used.
   */
  public void recordEffectUsage(int cl)
  {
    if (!_usedEffectTypes.contains(cl))
      {
        _usedEffectTypes.add(cl);
        for (var c : _clazzesWhoseCodeDependsOnUsedEffectTypes)
          {
            c._code = NO_SITE;
          }
      }
