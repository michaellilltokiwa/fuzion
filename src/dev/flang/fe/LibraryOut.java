...
class LibraryOut extends ANY
{
  ...
  void feature(Feature f)
  {
    ...
    _data.writeInt (argCount);      // NYI: PERFORMANCE: use better integer encoding
    _data.writeInt (n._id);         // NYI: PERFORMANCE: id /= 0 only if argCount = 0, so join these two values.
    pos(f.pos());
    ...
    // NYI: PERFORMANCE: Suppress output of inherits for fields, intrinsics, etc.?
    var i = f.inherits();
  ...
  void type(AbstractType t)
  {
    ...
    if (off >= 0)
      {
        _data.writeInt(-2);     // NYI: PERFORMANCE: optimization: maybe write just one integer, e.g., -index-2
        _data.writeInt(off);
      }
