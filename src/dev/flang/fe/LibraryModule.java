...
public class LibraryModule extends Module implements MirModule
{
  ...
  /**
   * NYI: ENHANCEMENT: Instead of using env var, create a new tool "fzdump" or similar to
  ...
  public SortedMap<FeatureName, AbstractFeature> declaredFeatures(AbstractFeature outer)
  {
    ...
    for (Module d : _dependsOn)
      {
        result.putAll(d.declaredFeatures(outer));  // NYI: BUG: handle equally named features from different modules
      }
    return result;
  }
    ...
    if (outer instanceof LibraryFeature lf)
      {
        for (var d : lf.declaredFeatures())
          {
            result.put(d.featureName(), d);  // NYI: BUG: handle equally named features from different modules
          }
      }
...
.2+| true     | 1      | int           | called feature f index
              | 1      | Type          | result type (NYI: CLEANUP: remove, redundant!)s
   | hasOpenArgList
        ...
        if (sf == null)
          {
            ...
            var ba = new byte[bb.limit()]; // NYI: PERFORMANCE: Would be better if SourceFile could use bb directly.
            bb.get(0, ba);
