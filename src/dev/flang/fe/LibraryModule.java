...
public class LibraryModule extends Module implements MirModule
{
   ...
   *
   * @param f the declaring feature
   */
  public SortedMap<FeatureName, AbstractFeature>declaredFeatures(AbstractFeature f)
  {
    ...
    for (var d : features(f))
      {
        result.put(d.featureName(), d);  // NYI: handle equally named features from different modules
      }
    for (Module d : _dependsOn)
      {
        result.putAll(d.declaredFeatures(f));  // NYI: handle equally named features from different modules
      }
    return result;
  }
...
--asciidoc--
  ...
  {
    return featureNextPos(moduleUniversePos());
  }
