...
public class AstErrors extends ANY
{
                               ...
                               List<FeatureAndOuter> targets)
  {
    ...
    error(pos,
          ...
          (outerLevels.size() == 1 ? "To solve this, you may rename one of these features." /* NYI: ENHANCEMENT: check if this case could actually happen,
  ...
  static void genericsMustBeDisjoint(SourcePosition pos, AbstractType t1, AbstractType t2)
  {
    error(pos,
          ...
          s(t1) + /* " at " + t1.pos().show() + */ "\n" +  // NYI: BUG: use pos before Types were interned!
          s(t2) + /* " at " + t2.pos().show() + */ "\n");
  }
  ...
  {
    // NYI: ENHANCEMENT: It would be nice to output the whole cycle here as part of the detail message
  ...
  static void failedToInferActualGeneric(SourcePosition pos, AbstractFeature cf, List<AbstractFeature> missing)
  {
    if (!any() || (cf != Types.f_ERROR && !missing.isEmpty()))
      {
        error(pos,
              "Failed to infer actual type parameters",  // NYI: ENHANCEMENT: give more detail here on type parameters and value arguments
