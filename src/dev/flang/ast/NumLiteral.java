...
public class NumLiteral extends Constant
{
  ...
  static enum ConstantType
  {
    ...
    ct_u32 (false, 4),
    ct_u64 (false, 8),
    // ct_f16 (11, 5),   -- NYI: UNDER DEVELOPMENT: support for f16
    ct_f32 (24, 8),
    ct_f64 (53, 11),
    ct_numeric(false, 4);
    // ct_f128 (113, 15),   -- NYI: UNDER DEVELOPMENT: support for f128
    // ct_f256 (237, 19),   -- NYI: UNDER DEVELOPMENT: support for f256
    ...
    if (m.signum() != 0)
      {
        ...
        if (e2 >= eSpecial)  // exponent too large
          {
            // NYI: CLEANUP: Determination of max float value is a little clumsy:
          ...
          { // denormalized
            ...
            if (m.signum() == 0)
              {
                // NYI: CLEANUP: Determination of min float value is a little clumsy:
