...
public class Intrinsics extends ANY
{
  ...
  static
  {
    ...
    put("fuzion.sys.fatal_fault0"      , (c,cl,outer,in) ->
        ...
        /* NYI: BUG: The C standard does not guarantee wrap-around semantics for signed types, need
    ...
    put("native_array", (c,cl,outer,in) -> A0.castTo("void *" /* NYI: CLEANUP: should be cast to array with element type cl._dfa._fuir.clazzActualGeneric(cl._cc, 0) */).ret());
  }
    ...
    else
      {
        ...
        result = CStmnt.seq(CExpr.call("fprintf",
                                        new List<>(new CIdent("stderr"),
                                                  CExpr.string("*** error: NYI: UNDER DEVELOPMENT: %s\n"),
                                                  CExpr.string(msg))),
                            CExpr.call("exit", new List<>(CExpr.int32const(1))));
      }
