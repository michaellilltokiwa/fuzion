...
public class Interpreter extends FUIRContext
{
   ...
   *
   *  param args -- NYI: UNDER DEVELOPMENT: command line args not supported yet
   */
  public void run()
  {
    try
  ...
  static Value tag(int choiceClazz, int valueClazz, Value val, int tagNum)
  {
    ...
    LValue slot = result.at(choiceClazz, 0); // NYI: UNDER DEVELOPMENT: needed? just result?
   ...
   *
   * NYI: UNDER DEVELOPMENT: Once static analysis detects use of uninitialized data, boxing this
                                     ...
                                     Value v)
  {
    if (PRECONDITIONS) require
      (!fuir().clazzIsRef(fclazz),
       ...
       v instanceof i8Value   ||  // NYI: UNDER DEVELOPMENT: what about u8/u16/..
